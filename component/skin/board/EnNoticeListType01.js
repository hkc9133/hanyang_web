...
const EnNoticeListType01 = ({content,pageChange,board,category}) => {
                    ...
                    {content.list.map((item) =>{
                        return (
                            <tr key={item.contentId} className={cx(item.isNotice ? "notice" : "")}>
                                ...
                                <td className={cx("txt_l")}><Link href={`/en/board/notice_en/view/${item.contentId}?${qs.stringify(router.query)}`}><a>{item.title}</a></Link></td>
...
export default EnNoticeListType01;
